...
// 版本号格式化：三位数字 → x.y.z
function _fmtVer(n: number): string {
  var s = String(n);
  if (s.length === 3) return s.charAt(0) + "." + s.charAt(1) + "." + s.charAt(2);
  if (s.length === 2) return "0." + s.charAt(0) + "." + s.charAt(1);
  return s;
}
...
export default function Screen(ctx: any): any {
  ...
  async function fetchInfo(srcIdx: number): Promise<any> {
  ...
  async function fetchPluginInfo() {
    ...
    if (si < 0 || si >= _infoUrls.length) { si = 0; }
    ...
    if (info.changelog && Array.isArray(info.changelog) && info.changelog.length > 0) {
      ...
      for (var i = 0; i < info.changelog.length; i++) {
        ...
        if (entry.details) blocks.push(String(entry.details));
      }
  ...
  function renderChangelogText(md: string): any {
    if (!md) return null;
  ...
  var versionCheckCard = ctx.UI.Card({ fillMaxWidth: true, containerColor: surfaceVariant }, [
    ctx.UI.Column({ padding: 16, spacing: 8 }, [
      ...
      ctx.UI.LazyRow({ spacing: 6 }, _infoLabels.map(function (label: string, idx: number) {
