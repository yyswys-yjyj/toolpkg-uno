...
exports.challengeWild4 = challengeWild4;
exports.acceptReveal = acceptReveal;
exports.abandonTurn = abandonTurn;
exports.GAME_DIR = "/storage/emulated/0/Download/Operit/cleanOnExit/uno";
...
function makeDeck() {
    ...
    const action = ["skip", "reverse", "draw2"];
    const deck = [];
    let seq = 0;
    ...
    for (let i = 0; i < 4; i++)
        deck.push({ color: "wild", value: "wild", id: "c" + seq++ });
    for (let i = 0; i < 4; i++)
        deck.push({ color: "wild", value: "wild4", id: "c" + seq++ });
    return deck;
}
...
// ---------- 新建游戏 ----------
function newGameState(gameId, firstTurn, rules) {
    ...
    const history = ["游戏开始，首张牌为 " + cardLabel(starter) + "，先手：" + (ft === "ai" ? "AI" : "用户")];
    let currentTurn = ft; // 默认先手出
...
// 判断某张牌能否打出
function canPlay(c, activeColor, top) {
    if (c.color === "wild")
        return true; // wild / wild4 总能出
    if (c.color === activeColor)
        return true;
    if (c.value === top.value)
        return true; // 数字或动作牌同名可叠
    return false;
}
// ---------- 抽牌 ----------
function draw(state, who) {
    let card;
...
// ---------- 通用出牌（who 指定玩家） ----------
function playCards(state, who, cardIds, chosenColor) {
    const top = state.pile[state.pile.length - 1];
    const hand = state.players[who];
    const opponent = who === "ai" ? "user" : "ai";
    ...
    for (const id of cardIds) {
        ...
        if (!c)
            return { ok: false, message: "卡牌不存在: " + id, won: false };
        if (seen.has(id))
            return { ok: false, message: "重复选择卡牌: " + id, won: false };
        seen.add(id);
        cardsToPlay.push({ ...c });
    }
    ...
    if (rev) {
        ...
        const sameKindPenalty = cardsToPlay.length > 0 && cardsToPlay.every((c) => rev.kind === "draw2" ? c.value === "draw2" : c.value === "wild4");
    ...
    // 记录 lastAction（质疑用）
    const origLast = { kind: "none", prevActiveColor, source: who, hadMatch: false };
    ...
    if (played.color === "wild") {
        ...
        if (chosenColor && ["red", "blue", "green", "yellow"].indexOf(chosenColor) >= 0) {
            ...
            const counts = { red: 0, blue: 0, green: 0, yellow: 0 };
            for (const c of state.players[who])
                if (c.color !== "wild")
                    counts[c.color]++;
            let bk = "red";
            let bn = -1;
    ...
    const kindForReveal = hasWild4 ? "wild4" : "draw2";
...
// 返回 { ok, message }
function challengeWild4(state, challenger) {
    ...
    const opponent = challenger === "ai" ? "user" : "ai"; // 出牌者
    // 判定
    if (la.hadMatch) {
        ...
        for (const c of preAll)
            state.players[challenger].push(c); // 原预取4张加入
        let extraDrew = 0;
        for (let i = 0; i < 2; i++) {
            if (draw(state, challenger))
                extraDrew++;
        } // 再罚2张
        state.pendingReveal = null;
        state.lastAction = { kind: "none", prevActiveColor: state.activeColor, source: challenger, hadMatch: false };
...
function acceptReveal(state, who) {
    const rev = state.pendingReveal;
    if (!rev)
        return { ok: false, message: "当前没有待确认的罚牌", ...{} };
    if (rev.target !== who) {
        return { ok: false, message: "当前不是 " + (who === "ai" ? "AI" : "用户") + " 确认罚牌", ...{} };
    }
    const drawn = rev.drawAbles || [];
    for (const c of drawn)
        state.players[who].push(c);
    state.pendingReveal = null;
...
// ---------- 通用弃牌/抽牌（who 指定玩家） ----------
function abandonTurn(state, who) {
    const top = state.pile[state.pile.length - 1];
    const opponent = who === "ai" ? "user" : "ai";
...
function colorText(c) {
    const m = { red: "红", blue: "蓝", green: "绿", yellow: "黄" };
    return m[c] || c;
}
