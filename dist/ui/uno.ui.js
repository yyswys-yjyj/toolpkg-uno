...
function _pickColor(hand, fallback) {
    ...
    var best = null, bn = -1;
    for (var k of ["red", "blue", "green", "yellow"]) {
        if (counts[k] > bn) {
            bn = counts[k];
            best = k;
        }
...
function Screen(ctx) {
    ...
    var chosen = {};
    try {
        Object.assign(chosen, JSON.parse(chosenState[0] || "{}"));
    }
    catch (e) { }
    ...
    var abandonDrawn = null;
    try {
        var ad = JSON.parse(abandonDrawnState[0] || "null");
        abandonDrawn = ad ? ad : null;
    }
    catch (e) {
        abandonDrawn = null;
    }
    ...
    // 罚牌确认后展示：确认后才允许看抽到的牌
    var revealedCards = null;
    try {
        var rc = JSON.parse(revealedCardsState[0] || "");
        revealedCards = rc && rc.length > 0 ? rc : null;
    }
    catch (e) {
        revealedCards = null;
    }
    ...
    var stackDone = null;
    try {
        var sd = JSON.parse(stackDoneState[0] || "");
        stackDone = sd ? sd : null;
    }
    catch (e) {
        stackDone = null;
    }
    ...
    var wildPick = null;
    try {
        var wp = JSON.parse(wildPickState[0] || "null");
        wildPick = wp ? wp : null;
    }
    catch (e) {
        wildPick = null;
    ...
    function toggleChosen(cardId) {
        var nc = {};
        try {
            Object.assign(nc, JSON.parse(chosenState[0] || "{}"));
        }
        catch (e) { }
        nc[cardId] = !nc[cardId];
        chosenState[1](JSON.stringify(nc)); // setState → 重绘
    }
    async function saveResult(msg) {
        ...
        try {
            await Tools.Chat.sendMessage(msg, chatId, undefined, undefined, { runtime: "main" });
        }
        catch (e) { }
    }
    ...
    function syncLocalGame(st) {
        try {
            var d = {};
            try {
                d = JSON.parse(dataState[0] || "{}");
            }
            catch (e) {
                d = {};
            }
            if (st)
                d.game = JSON.stringify(st);
            d.gameLoaded = true;
    ...
    async function handlePlay() {
        if (!isUserTurn || submitted || isAbandoning)
            return;
        var chosenIds = [];
        for (var k in chosen) {
            if (chosen[k])
        ...
        var st = { players: { ai: [], user: [] }, pile: [], deck: [], activeColor: "", currentTurn: "user", winner: null };
        try {
        ...
        for (var wi = 0; wi < chosenIds.length; wi++) {
            var wc = st.players.user.find(function (h) { return h.id === chosenIds[wi]; });
            if (wc && wc.color === "wild") {
                pickedWild = true;
                break;
            }
    ...
    async function confirmWildColor() {
        if (!isPickingWild || !wildPick || !wildPick.ids)
            return;
        ...
        if (mode === "stack") {
            ...
            if (!revPending || revPending.target !== "user") {
                errorMsgState[1]("当前不是待确认罚牌状态");
                wildPickState[1]("null");
                wildColorState[1]("");
                return;
            }
            if (!st.rules || !st.rules.stacking) {
                errorMsgState[1]("未开启叠加规则，无法接招");
                wildPickState[1]("null");
                wildColorState[1]("");
                return;
            }
            var sc = (st.players.user || []).find(function (h) { return h.id === ids[0]; });
            if (!sc || sc.value !== "wild4") {
                errorMsgState[1]("只能接招「同类 +4」");
                wildPickState[1]("null");
                wildColorState[1]("");
                return;
            }
        ...
        try {
            await ctx.callTool("write_file", { path: path, content: JSON.stringify(st) });
        }
        catch (e) { }
    ...
    async function confirmReveal() {
        ...
        if (!st.pendingReveal || st.pendingReveal.target !== "user")
            return;
    ...
    async function stackReveal(cardId) {
        if (submitted)
            return;
        errorMsgState[1]("");
        var path = "/storage/emulated/0/Download/Operit/cleanOnExit/uno/" + gameId + ".json";
        ...
        // 该牌须与罚牌同类型
        var c = (st.players.user || []).find(function (h) { return h.id === cardId; });
        if (!c) {
            errorMsgState[1]("卡牌不存在");
            return;
        }
        var sameKind = revPending.kind === "draw2" ? c.value === "draw2" : c.value === "wild4";
        if (!sameKind) {
            errorMsgState[1]("只能接招「同类罚牌」");
            return;
        }
    ...
    function localPlay(st, ids, chosenColor) {
        ...
        for (var i = 0; i < ids.length; i++) {
            var id = ids[i];
            var c = hand.find(function (h) { return h.id === id; });
            if (!c)
                return { ok: false, message: "卡牌不存在: " + id };
            if (seen[id])
                return { ok: false, message: "重复选择卡牌: " + id };
            seen[id] = true;
            cards.push({ ...c });
        }
        if (cards.length > 1) {
        ...
        for (var id2 of ids)
            idSet[id2] = true;
        st.players.user = hand.filter(function (h) { return !idSet[h.id]; });
        for (var c2 of cards)
            st.pile.push(c2);
        var played = cards[0];
    ...
    function renderAttachBox() {
        return ctx.UI.Column({ spacing: 4, fillMaxWidth: true }, [
            ...
            ctx.UI.TextField({
                ...
                onValueChange: function (newVal) { attachMsgState[1](newVal); },
    ...
    if (isUserPenalized) {
        ...
        if (stackingOn && myHand && myHand.length) {
            for (var si = 0; si < myHand.length; si++) {
                var sc = myHand[si];
                if (pending.kind === "draw2" && sc.value === "draw2")
                    stackable.push(sc);
                else if (pending.kind === "wild4" && sc.value === "wild4")
                    stackable.push(sc);
            }
        }
    ...
    if (isPickingWild) {
        ...
        var wildIds = wildPick && wildPick.ids;
        if (wildIds && wildIds.length) {
            var wf = myHand.find(function (h) { return h.id === wildIds[0]; });
            if (wf)
                wildCardLabel = _cardLabel(wf);
        }
